...
const xpNeeded = (level) => {
  return Math.round(Math.pow(level, 2)+20 + (level+1)*20 + Math.sqrt(level) + Math.pow(1.0240191618, level));
}
...
async function clearDatabase() {
	await db.set("accounts", {}).then(console.log("Database Cleared"))
}
	...
	ws.on("message", (data) => {
		...
		if (pendingClient.account != false) {
			if (msg.type === "join") {
				...
				if (!fail) {
					clients[clientId] = ws;
					players[clientId] = new Player(clientId, pendingClient.account, pendingClient.guest);
					const peyload = {
						type: "init",
						selfId: clientId,
						config: {
